...
```
## 6. Estandarizar residuos (z-scores ocupando la media y SD de los CN)

Se convierten los residuos a puntajes z para asegurar comparabilidad de escalas.

```{r zscore-control}
# 1) Calcula medias y ds sólo en controles
controls <- data_resid %>% filter(clinical_diagnosis == "CN")

# Vector de ítems NPI
npi_vars <- names(data_resid) %>% str_subset("^npi_.*sev$")
