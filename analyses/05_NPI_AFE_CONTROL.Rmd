...
data_limpia2 <- read_csv("../data/data_original/data_limpia_2.csv")
```
...
## 5. Estandarizar variables (z-scores ocupando la media y SD de los CN)

Se convierten a puntajes z para asegurar comparabilidad de escalas.

```{r zscore-control}
# 1) Calcula medias y ds sólo en controles
controls <- data_limpia2 %>% filter(clinical_diagnosis == "CN")

# Vector de ítems NPI
npi_vars <- names(data_limpia2) %>% str_subset("^npi_.*sev$")


# Identificar variables con SD = 0 en controles (solo para npi_vars)
vars_sd_cero <- npi_vars[sapply(controls[npi_vars], sd, na.rm = TRUE) == 0]

# Mostrar resultados
print(vars_sd_cero)
# hay dos variables que no tienen sd en los controles "npi_delsev"  "npi_hallsev" por lo que no se ralizó el z-score en ellas

# 2) Saca medias y ds de controles
ctrl_stats <- controls %>%
  summarise(across(all_of(npi_vars-vars_sd_cero),
                   list(mean = ~ mean(.x, na.rm=TRUE),
                        sd   = ~ sd(.x,   na.rm=TRUE)),
                   .names = "{.col}_{.fn}"))

# 3) Aplica el z-score basado en ctrl_stats
data_z <- data_limpia2 %>%
  mutate(across(
    all_of(npi_vars),
...
# Comprueba
head(select(data_z, all_of(npi_vars)), 10)
```

## 6. Pruebas de adecuación factorial
...
**Prueba de Bartlett:** χ²(66) = **`r bartlett_res$chisq`** (df = **`r bartlett_res$df`**), p = **`r format(bartlett_res$p.value, digits=2)`**. Este resultado significativo indica que la matriz de correlaciones **no** es una matriz identidad, lo que confirma que existe suficiente correlación entre los ítems para justificar un análisis factorial.

**Índice KMO:** El valor global de MSA = **`r round(kmo_res$MSA,2)`**, se considera *muy bueno*, por encima del umbral de 0.80. Además, los valores individuales de MSA para cada ítem (rango 0.83–0.91) superan el criterio mínimo de 0.60, lo que muestra que cada ítem aporta adecuadamente a la factorización.
...
## 7. Determinar número de factores (Análisis paralelo)
...
El resultado indica que **el número óptimo de factores es `r nf`** (según fa.parallel, aquel donde el eigenvalor real supera al de los datos simulados).. Esto significa que, tras ajustar por edad, escolaridad y sexo y estandarizar los residuos, cuatro factores principales capturan la estructura subyacente de las variables de severidad del NPI, superando a los eigenvalores esperados bajo aleatoriedad.

## 8. Análisis Factorial Exploratorio
...
- **Ajuste del modelo:**
  - **RMSR** = **`r round(ef$rms,2)`** (muy cercano a 0) indica excelentes ajustes entre la matriz observada y la recreada.
  - **RMSEA** = **`r round(ef$RMSEA[1],3)`** (IC 90%: **`r round(ef$RMSEA[2],3)`**–**`r round(ef$RMSEA[3],3)`**) sugiere buen ajuste (valores <0.06 aceptables).
...
## 9. Obtener puntuaciones y compuestos
...
### 9.2 Consistencia interna: Cronbach’s Alpha
...
# Definir rutas
dir_out  <- "../outputs/05/npi_afe_control"
...
write_xlsx(
  cronbach_res,
  path = file.path(dir_out, "05_npi_afe_cronbach_control")
)
...
Factor 1 (Afecto depresivo/ansiedad) agrupó tres ítems relacionados con el estado de ánimo: depresión, ansiedad y apatía. Su consistencia interna fue aceptable (α = **`r cronbach_res$Alpha[1]`**)).

Factor 2 (Sueño y apetito) combinó apatía (nuevamente), alteraciones del sueño y cambios en el apetito, reflejando desregulación de patrones básicos de descanso y alimentación (α = **`r cronbach_res$Alpha[2]`**).

Factor 3 (Agitación/desinhibición) incluyó agitación motora, euforia, desinhibición e irritabilidad, configurando un núcleo de conductas exteriorizadas (α = **`r cronbach_res$Alpha[3]`**).

Factor 4 (Sintomatología psicótica) reunió delirios y alucinaciones, correspondientes al dominio psicótico (α = **`r cronbach_res$Alpha[4]`**).
...
## 10. Comparación Paramétrica de puntuaciones factoriales entre grupos 
...
- **Factor 1** los controles sanos (NC) presentan puntuaciones centrales cercanas a 0, los pacientes con AD se sitúan ligeramente por encima, y los de bvFTD muestran los valores más altos. El ANOVA (p < 2e-16) indica diferencias significativas entre los tres grupos, con un gradiente NC < AD < bvFTD.
...
```
### 10.2 Pruebas Tukey post-hoc entre grupos 
...
### 10.3 Tabla de resultados paramétricos
...
### 10.4 Evaluación de supuestos (normalidad y homocedasticidad de los residuos)
