...
```{r zscore}
data_z <- data_resid %>%
  mutate(across(all_of(npi_vars), ~ scale(.x, center = TRUE, scale = TRUE)))
```
...
## ¡¡¡¡¡¡ver warning de aqui hacia abajo!!!!!!

## 11. Comparación de puntuaciones factoriales entre grupos
...
```{r posthoc-factor, warning=FALSE}
library(emmeans)
posthoc_list <- list()
for(f in paste0("Factor", 1:4)) {
...
### 11.3 Evaluación de supuestos

```{r check-assumptions, warning=FALSE}

# Iteramos por cada factor para testear supuestos
assumption_results <- map(paste0("Factor",1:4), function(f) {
  mod <- aov(as.formula(paste(f, "~ clinical_diagnosis")), data = data_scores)
  # Shapiro-Wilk en residuos
  sw <- shapiro_test(residuals(mod))
  # Levene sobre varianzas
  lv <- levene_test(as.formula(paste(f, "~ clinical_diagnosis")), data = data_scores)
  list(factor = f, shapiro = sw, levene = lv)
})

assumption_results
```

- **Si en algún factor** `shapiro$p < .05` indica que los residuos no siguen normalidad, y si **`levene$p < .05`** indica heterogeneidad de varianzas.  
- Cuando fallen estos supuestos, mejor usar **`kruskal.test()`** seguido de **`dunn_test()`** (por ejemplo, `data_scores %>% dunn_test(Factor1 ~ clinical_diagnosis, p.adjust.method="bonferroni")`), en lugar de ANOVA + Tukey.

Así tendrás la certeza de que tus post-hoc son válidos o, de lo contrario, sabrás cuándo debes usar métodos no paramétricos.

### 11.4 Métodos no paramétricos
Si **shapiro_p < .05** o **levene_p < .05**, usar Kruskal-Wallis y Dunn:

```{r nonparametric}
library(rstatix)
for(f in paste0("Factor",1:4)) {
  kw <- kruskal_test(as.formula(paste(f, "~ clinical_diagnosis")), data = data_scores)
  cat(f, ": Kruskal-Wallis p=", kw$p, "\n")
  if(kw$p < .05) {
    print(dunn_test(data_scores,
                    as.formula(paste(f, "~ clinical_diagnosis")),
...
- **Factor 1** y **Factor 2**: las comparaciones post-hoc mostraron diferencias significativas en los tres pares (HC vs AD, AD vs bvFTD, HC vs bvFTD; p.adj \< .001), confirmando el gradiente HC < AD < bvFTD.

- **Factor 3**:
  - **HC vs AD:** p.adj = .1896 (no significativo), lo que indica puntuaciones similares entre controles y AD.
  - **HC vs bvFTD:** p.adj \< .0001 (****), bvFTD > HC.
...
- **Factor 4**:
  - **HC vs AD:** p.adj = .6451 (ns), sin diferencia entre controles y AD.
  - **HC vs bvFTD:** p.adj \< .0001 (****), bvFTD > HC.
  - **AD vs bvFTD:** p.adj \< .0001 (****), bvFTD > AD.
  
  Indica que la sintomatología psicótica (Factor 4) está elevada en bvFTD, pero no en AD comparado con controles.

Estas conclusiones complementan el ANOVA global y muestran un patrón más matizado: mientras que los factores 1 y 2 discriminan todos los pares de grupos, los factores 3 y 4 distinguen principalmente bvFTD de AD y de HC.
